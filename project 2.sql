...
CREATE OR REPLACE FUNCTION datediff(unit TEXT, start_date DATE, end_date DATE)
RETURNS INTEGER AS $$
BEGIN
  -- Se a unidade for "day", subtrai diretamente as datas
  -- (em PostgreSQL, subtrair datas retorna a quantidade de dias)
  IF unit = 'day' THEN
    RETURN end_date - start_date;

  -- Se a unidade for "month", calcula a diferença total em meses
  -- Primeiro calcula os anos de diferença e converte em meses
  -- Depois soma a diferença entre os meses de cada data
  ELSIF unit = 'month' THEN
    RETURN (date_part('year', end_date) - date_part('year', start_date)) * 12 +
           (date_part('month', end_date) - date_part('month', start_date));

  -- Se a unidade for "year", calcula a diferença apenas entre os anos
  ELSIF unit = 'year' THEN
    RETURN date_part('year', end_date) - date_part('year', start_date);

  -- Se nenhuma das unidades for reconhecida, retorna NULL
  ELSE
    RETURN NULL;
  END IF;
END;
...
select
	case
		when datediff('year', birth_date, '2021-01-01') < 20 then '0-20'
		when datediff('year', birth_date, '2021-01-01') < 40 then '20-40'
		when datediff('year', birth_date, '2021-01-01') < 60 then '40-60'
		when datediff('year', birth_date, '2021-01-01') < 80 then '60-80'
